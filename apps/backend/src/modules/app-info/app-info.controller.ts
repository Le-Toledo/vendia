...
export class AppInfoController {
  ...
  privacy(@Res() response: Response) {
    ...
    response
      .type('html')
      .send(
        `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Privacidade — VendAI</title></head><body><main><h1>${escape(policy.title)}</h1><p>Versão ${policy.version}</p>${policy.sections.map((section) => `<section><h2>${escape(section.title)}</h2><p>${escape(section.text)}</p></section>`).join('')}</main></body></html>`,
      );
  }
}
